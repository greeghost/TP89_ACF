theory tp89proof
  imports Main table "~~/src/HOL/Library/Code_Target_Nat" tp89
begin
...
type_synonym ligneBdd= "betterNat * betterNat * bool * bool "
...
lemma modify: "i1 \<noteq> i2 \<longrightarrow> assoc i2 (modify i1 val t) = assoc i2 t "
  apply (induct t)
   apply auto
  done

lemma " (id1 \<noteq> id2) \<longrightarrow> assoc id1 (traiterMessage (Pay id2 m) tt) = assoc id1 tt "
  oops
lemma " (id1 \<noteq> id2) \<longrightarrow> assoc id1 (traiterMessage (Ack id2 m) tt) = assoc id1 tt "
  apply (case_tac "assoc id1 tt")
   prefer 2
   apply (case_tac "assoc id1 (traiterMessage (Ack id2 m) tt)")
    prefer 2
  apply simp

lemma lemme1_3: " (id1 \<noteq> id2) \<longrightarrow> assoc id1 (traiterMessage (Cancel id2) tt) = assoc id1 tt "
  apply auto
  using modify apply metis
  done
...
lemma "assoc id1 (traiterMessage (Cancel id1) tt) = Some((Undeclared, Undeclared, True, False))"
  oops
...
fun validOne:: "ligneBdd \<Rightarrow> bool" where 
  "validOne (amc, amm, ccl, vld) = (if (vld) then True else False) "

fun valide::" transBdd \<Rightarrow> bool" where
  "valide [] = False " |
  "valide [(id1, x)] = validOne x " |
  "valide ((id1,x)#xs) = ((validOne x) \<and> (valide xs)) "

(* Lemme 3 *)
(* Si une bdd est valide, alors quelque soit la ligne obtenue par la fonction assoc, celle-ci est valide *)

lemma " valide tt \<longrightarrow> assoc id1 tt = Some(a) \<longrightarrow>  validOne(a)"
  oops 
